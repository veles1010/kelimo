...
class _AchievementCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final isPerfectQuiz = achievement.id == 'perfect_quiz';
    final accent = isPerfectQuiz
        ? Colors.amber.shade700
        : Theme.of(context).colorScheme.secondary;
    return Opacity(
      opacity: isUnlocked ? 1 : 0.74,
      child: GlassSurface(
        key: ValueKey('achievement-${achievement.id}'),
        enableBlur: false,
        showHighlight: isUnlocked,
        showShadow: isUnlocked,
        padding: EdgeInsets.zero,
        child: Card(
          color: isUnlocked
              ? accent.withValues(alpha: isPerfectQuiz ? 0.08 : 0.04)
              : Colors.transparent,
          surfaceTintColor: Colors.transparent,
          child: Padding(
